...
class UnicodeBlock(object):
    def __init__(self, index, block_data):
        self.index = index
        self.start, self.end, self.name = block_data
    ...
    def get_char_list(self):
        ...
        """
        data = []
        for no in xrange(int(self.start), int(self.end)):
            char = unichr(no)
            char_code = "%X" % no
...
class _UnicodeBlockData(object):

    def __init__(self):
        self.block_list = []
        self.block_data = {}
        for index, block in enumerate(BLOCKS):
            ub = UnicodeBlock(index, block)
            self.block_list.append(ub)
            self.block_data[ub.slug] = ub

    def __getitem__(self, block_slug):
        return self.block_data[block_slug]

    def get_first_block(self):
        return self.block_list[0]

    def get_next_block(self, unicode_block):
        current_index = unicode_block.index
        try:
            return self.block_list[current_index + 1]
        except KeyError:
            return None

    def get_prior_block(self, unicode_block):
        current_index = unicode_block.index
        if current_index == 0:
            return None
        return self.block_list[current_index - 1]
    ...
    def get_choices(self):
        ...
        for unicode_block in self.block_list:
            block_choices.append([
                unicode_block.slug,
                "0x%04X-0x%04X - %s" % (
                    unicode_block.start, unicode_block.end, unicode_block.name
                )
            ])
